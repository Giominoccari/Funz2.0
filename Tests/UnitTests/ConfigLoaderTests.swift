...
struct ConfigLoaderTests {
    ...
    func loadValidConfig() throws {
        ...
        #expect(config.pipeline.scoringWeights.weather.rain14d == 0.55)
        #expect(config.pipeline.scoringWeights.weather.temperature == 0.45)
        #expect(config.pipeline.scoringWeights.humidityMultiplierMin == 0.4)
    ...
    func weatherWeightsSum() throws {
        ...
        let sum = w.rain14d + w.temperature
        #expect(abs(sum - 1.0) < 0.001)
    }
}
